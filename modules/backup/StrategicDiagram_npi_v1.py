...
def split_column_into_lists(column, splitter):
    ...
    for i in range(len(column)):
        try:
            ...
            if isinstance(column[i], str): # some columns consist of rows with list while others with string
                column[i] = str(column[i]).split(splitter)
                for j in range(len(column[i])):
                    column[i][j] = column[i][j].strip()
            else:
                column[i] = column[i]          
    return column
...
def npiFirm_to_alliance(data):
    ...
    #2 npi of a focal firm
        focal = npi_firm(data2["focal"][i], firm_list_full, "focal")
        # focal = multi.multiprocess(data2["focal"][i], npi_firm, "focal")
        npi_focal.append(focal)
    #3 npi of a partner firm
        partner = npi_firm(data2["partner"][i], firm_list_full, "partner")
